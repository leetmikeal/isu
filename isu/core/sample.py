...
class Sample():
    ...
    def __crop_image(self, image_raw_list, class_raw_list, count, size):
        ...
        while(c < count):
            ...
            crop_image = image[range_x:range_x +
                               size[0], range_y:range_y +
                               size[1], range_z:range_z +
                               size[2], :]
            crop_class = cl[range_x:range_x +
                            size[0], range_y:range_y +
                            size[1], range_z:range_z +
                            size[2], :]
    ...
    def split(self, train_count, val_count, val_biased=False):
        if val_biased:
            ...
            stratSplit = StratifiedShuffleSplit(
                n_splits=1, test_size=val_count)
            for sample_idxs, val_idxs in stratSplit.split(
                    self.image_list, self.class_list):
