...
def ensure_exist_save_dir(path):
    '''
    指定したファイルパスの親ディレクトリが確実に存在するようにする
...
def generate_common_model(save_model_path, save_image_path=None):  # 以後一番いじられるところ
    ...
    # Arguments
        save_model_path: saving model file path
        save_image_path: summary graph image file path
    ...
    # number_class = input_file_data["parameter_model_train"]["num_classes"]
    ####
...
def save_network(model, path):
    """
    ネットワーク構造モデルをファイルに保存する。

    # Arguments:
        model: ネットワーク構造オブジェクト
        path: ファイル保存パス
...
def save_graph(model, path):
    """
    モデルのサマリーグラフを画像ファイルとして保存する。

    # Arguments:
        model: ネットワーク構造オブジェクト
        path: 保存先の画像ファイル保存パス(*.png)
...
def get_shape_in(shape_in):
    """
    Convert command line argument input shape to each variables

    # Arguments:
        shape_in: like 224,224,3
...
def create(args):
    ...
    # To compile is nessesary for to pb transportation and C#(tensorflow-sharp) loading
